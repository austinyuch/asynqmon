...
docker run --rm \
    --name asynqmon \
    -p 8080:8080 \
    asynqmon   # locally built (make docker); remote registry deferred
```
...
# with a docker image
docker run asynqmon --help   # after: make docker (or podman build -t asynqmon .)
```
...
docker run --rm \
    --name asynqmon \
    -p 3000:3000 \
    asynqmon --port=3000 --redis-addr=host.docker.internal:6380
...
docker run --rm \
    ...
    --network dev-network \
    -p 8080:8080 \
    asynqmon --redis-addr=dev-redis:6379
```
