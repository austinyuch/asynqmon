...
git config core.hooksPath githooks   # 啟用 canonical local CI pre-push gate
```
...
| Local supply-chain CI | `scripts/local-ci.sh` + pre-push:Go build/vet/race、UI frozen install/lint/test/build、Semgrep、CycloneDX、Trivy CVE、CISA KEV exact-match、govulncheck;證據在 ignored `.local-ci/`;`.security/vex.json` 為 tracked narrow disposition | SPEC-006 shift-left;missing/stale/tampered evidence fail closed |
| 其他 | `FORK.md`、`AGENTS.md`、`.agents/{skills,specs}/`、`docs/{manual,review}/`(live-evidence 文件,guides 可再生) | 團隊維運工具 |
