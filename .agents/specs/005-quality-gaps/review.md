...
### REQ-QG-002(eslint)
✅ eslint 9 flat config,`yarn lint` **0 errors / 0 warnings**(原 `SplitButton` render-time ref warning 由 CR-2026-07-30-003 以 MUI lazy anchor contract 修正)。規則調整逐條記理由(config 註解 + design.md):no-explicit-any off(API 轉接層)、exhaustive-deps/refs warn(行為保留)、no-case-declarations/no-useless-assignment off(legacy reducer 模式)。真修:prototype-builtins、未用 imports/params/catch(含誤刪有用 catch binding 一次,vitest+tsc 攔回——Q-R3 防線生效)。CI lint+vitest step 入 build job。
